...
class Google implements PluginContract
{
    ...
    protected $httpOpt = [];
    const API = 'https://www.google.co.id/search';
    const RULES = [
      'title' => ['h3','text'],
    //  'titleVideo' => ['.KiGY3d','html'],
      'link' => ['h3>a','href']
    ];
    const RANGE = '.g';
    ...
    public function getCount()
    {
		...
		$text = explode('(',$text);
		$text = $text[0];
       // if(preg_match('/[\d,]+/',$text,$arr)) $output_array=implode("",$output_array[0]);
        if(preg_match_all("/([0-9]+[.]*)/", $text, $arr))
        {
            $arr = implode('',$arr[0]);
			$count = str_replace(',','',$arr);
			$count = str_replace('.','',$arr);
